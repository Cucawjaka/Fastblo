...
class AuthService:
    ...
    async def change_password(self, user_id: int, data: ChangePassword) -> dict[str, str]:
        ...
        updated_token = await self._token_dao.update_record(
            values=RefreshTokenSave(
                refresh_token=tokens_dict["refresh_token"],
                user_id=user_from_db.id
            ),
        ...
        if not updated_token:
            await self._token_dao.add_one_record(values=RefreshTokenSave(
                refresh_token=tokens_dict["refresh_token"],
                user_id=user_from_db.id
            ))
        return tokens_dict
    ...
    async def refresh_tokens(self, refresh_token) -> dict[str, str]:
        ...
        if not token_from_db:
            raise ValueError("Обновление токенов невозможно")
